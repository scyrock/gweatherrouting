...
pyinstaller --onefile --hidden-import=gi --collect-submodules=gi --add-data "data/:gweatherrouting/data" --add-data "gtk/:gweatherrouting/gtk" --name "$APP_NAME" --additional-hooks-dir=. main.py
cd ..
...
NO_STRIP=true DEPLOY_GTK_VERSION=3 ./linuxdeploy-x86_64.AppImage --appdir "$APP_DIR" --plugin gtk

# 7. Generate the AppImage
cp "AppRun" "$APP_DIR/"
